...
public class MainActivity extends BaseActivity implements EMEventListener {
	...
	private TabMain TabMain;//首页
	
	private ChatAllHistoryFragment chatHistoryFragment;//聊天
	
	private TabCabinet TabCabinet;//酒柜
	
		private TabDiscover TabDiscover; //发现
	
	private TabMy TabMy; //我的
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null && savedInstanceState.getBoolean(Constant.ACCOUNT_REMOVED, false)) {
			...
			/// 防止被T后，没点确定按钮然后按了home键，长期在后台又进app导致的cr 三个fragment里加的判断同理
			finish();
			startActivity(new Intent(this, LoginActivity.class));
			return;
		}
		...
		TabMain = new TabMain();
		TabCabinet = new TabCabinet();
		TabDiscover = new TabDiscover();
		TabMy = new TabMy();
		fragments = new Fragment[] {TabMain, chatHistoryFragment ,TabCabinet,TabDiscover,TabMy};
		findViewById(R.id.main_bottom).setVisibility(View.VISIBLE);
		// 添加显示第一个fragment
		getSupportFragmentManager().beginTransaction().add(R.id.fragment_container, chatHistoryFragment).hide(chatHistoryFragment)
				.add(R.id.fragment_container, contactListFragment,"contactListFragment").hide(contactListFragment)
				.add(R.id.fragment_container, settingFragment,"settingFragment").hide(settingFragment)
				.add(R.id.fragment_container, TabMain).show(TabMain)
				.commit();
//		//获取Intent
//		Intent intent = getIntent();
//		if(intent.getExtras() != null){
//		//直接通过intent携带的Bundle数据包中的数据
//		int x = intent.getExtras().getInt("index");
//		
//		switch (x) {
//		case 1:
...
//			
//		case 2:
//			getSupportFragmentManager().beginTransaction().add(R.id.fragment_container,settingFragment)
//			.hide(TabMy)
...
//		}
	...
	private void initView() {
		...
		unreadAddressLable = (TextView) findViewById(R.id.unread_address_number1);
		mTabs = new Button[5];
		mTabs[0] = (Button) findViewById(R.id.btn_index);
		mTabs[1] = (Button) findViewById(R.id.btn_conversation);
		mTabs[2] = (Button) findViewById(R.id.btn_winebox);
	...
	public void onTabClicked(View view) {
		...
		if (currentTabIndex != index) {
			...
			trx.hide(fragments[currentTabIndex]).hide(contactListFragment).hide(settingFragment);
			if (!fragments[index].isAdded()) {
				if(index == 4){
					trx.add(R.id.fragment_container, fragments[index],"tabMy");
				}else{
